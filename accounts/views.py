...
def signup(request): 
    ...
    - return : 성공 -> home로 이동 / 실패 -> signup.html 화면 표시
    """
    if request.method == 'POST': #사용자가 화면에서 '회원가입' 버튼을 눌러 데이터를 전송한 경우(처리 시작)
        if request.POST['password'] == request.POST['confirm']:
            ...
            if User.objects.filter(username=username).exists():
                return render(
                    request, 'accounts/signup.html', {'message' : '이미 사용 중인 아이디입니다.'}
                )

            if Profile.objects.filter(email=email).exists():
                return render(
                    request, 'accounts/signup.html', {'message' : '이미 사용 중인 이메일입니다.'}
                )
...
def login(request):
    ...
    - return : 성공 -> home로 이동 / 실패 -> login.html 화면 표시
    """
    if request.method == 'POST': #사용자가 화면에서 '로그인' 버튼을 눌러 데이터를 전송한 경우(처리 시작)
        ...
        if writer is not None:  #로그인 성공한 경우
            ...
            return redirect('main:home') #메인:첫화면 주소 반환
        else:
            return render(request, 'accounts/login.html') #accounts/templates/accounts/login.html 로 이동
